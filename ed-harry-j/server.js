...
// REVIEWEDED: These are routes for requesting HTML resources.
app.get('/new-article', (request, response) => {
  response.sendFile('new.html', { root: './public' });
});


// REVIEWEDED: These are routes for making API calls to enact CRUD operations on our database.
app.get('/articles', (request, response) => {
  client.query(`SELECT authors.author_id, articles.author_id, authors.author, authors.author_url, articles.body, articles.category, articles.published_on, articles.title FROM authors INNER JOIN articles ON authors.author_id = articles.author_id;`)
    .then(result => {
...
app.post('/articles', (request, response) => {
  ...
  client.query(SQL, values,
    function(err) {
      ...
      // REVIEWEDED: This is the second query
      queryTwo();
    }
  )
